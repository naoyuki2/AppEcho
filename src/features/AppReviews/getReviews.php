...
function getReviews($appId)
{
    global $pdo;
    $sql = $pdo->prepare('
            SELECT review.id, review.app_id, review.content, review.star, review.post_date, review.user_id, tag.name AS tag_name,tag.color AS tag_color ,review.tag_id
            FROM review
            INNER JOIN tag
...
function getUser()
{
    ...
    $sql = $pdo->query("
            SELECT id, name, icon_image_url, isAnonymous
            FROM user
        ");
